...
class PacienteRegistroPublicoSerializer(serializers.Serializer):
    """
    [SPRINT 1 - T015] Flujo de Autogesti├│n de credenciales por el Paciente.
    """
    ...
    def validate_clinica_id(self, value):
        ...
        try:
            ...
            raise serializers.ValidationError("La cl├¡nica seleccionada no existe.")

    def validate_email(self, value):
        if Usuario.objects.filter(email=value).exists() or Usuario.objects.filter(username=value).exists():
            raise serializers.ValidationError("Este correo ya est├í registrado.")
        return value

    def validate_ci(self, value):
        if Paciente.objects.filter(ci=value).exists():
            raise serializers.ValidationError("Esta C├®dula de Identidad ya est├í registrada.")
        return value
    ...
    def create(self, validated_data):
        ...
        paciente = Paciente.objects.create(
            ...
            telefono=telefono,
            motivo_consulta="Registro M├│vil"
        )
        
        # Crear NotificacionPush de bienvenida
        hora_actual = timezone.localtime(timezone.now()).strftime("%H:%M")
        titulo_notif = "┬íBienvenido a PsicoSystem!"
        mensaje_notif = f"Hola {usuario.username}, tu cuenta ha sido creada exitosamente a las {hora_actual}. ┬íNos alegra tenerte aqu├¡!"
