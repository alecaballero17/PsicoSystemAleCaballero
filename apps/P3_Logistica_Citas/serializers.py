...
class CitaSerializer(serializers.ModelSerializer):
    ...
    clinica_nombre = serializers.CharField(source='paciente.clinica.nombre', read_only=True)
    psicologo_nombre = serializers.SerializerMethodField(read_only=True)

    class Meta:
        model = Cita
        fields = [
            "id", "paciente", "paciente_nombre", "clinica_nombre",
            "psicologo", "psicologo_nombre",
            "fecha_hora", "motivo", "estado", 
            "estado_pago", "monto", "numero_ficha", "codigo_qr"
        ]
