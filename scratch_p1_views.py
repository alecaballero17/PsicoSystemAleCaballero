...
# ==========================================================================
# Endpoint /api/auth/me/ ÔÇö Verificaci├│n de sesi├│n JWT (Anti-hu├®rfano)
# ==========================================================================
class MeAPIView(APIView):
    """GET: Devuelve los datos del usuario autenticado. Usado por el frontend para verificar sesi├│n."""
...
class CustomTokenObtainPairView(TokenObtainPairView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        # Login exitoso ÔåÆ resetear contador
        cache.delete(cache_key)
        return response


# ==========================================================================
# Registro de psic├│logo ÔÇö vista de plantilla (formulario HTML)
# ==========================================================================
...
def registrar_usuario_view(request):
    if request.method == "POST":
        ...
        logger.warning("Registro inv├ílido: %s", form.errors)
    else:
...
class RegistroPsicologoAPIView(APIView):
    """POST p├║blico: alta cl├¡nica + usuario psic├│logo; devuelve tokens JWT (m├│vil / Postman)."""
...
class ChangePasswordAPIView(APIView):
    """PUT: Cambiar la contrase├▒a del usuario autenticado."""
    permission_classes = [IsAuthenticated]
    ...
    def put(self, request):
        ...
        if not new_password or len(new_password) < 8:
            return Response({"detail": "La contrase├▒a debe tener al menos 8 caracteres."}, status=status.HTTP_400_BAD_REQUEST)
...
class PlanesListAPIView(APIView):
    """GET p├║blico: devuelve el cat├ílogo de planes configurados en el modelo Cl├¡nica, incluyendo PRECIOS para Stripe."""
    permission_classes = [AllowAny]

    def get(self, request):
        # Cat├ílogo de precios y caracter├¡sticas de los planes SaaS
        catalogo = [
            {"id": "Basico", "nombre": "Plan B├ísico (Gratis)", "precio": 0.00, "duracion_dias": 365, "beneficios": "Gesti├│n de 1 agenda, soporte por email."},
            {"id": "Profesional", "nombre": "Plan Profesional", "precio": 49.99, "duracion_dias": 30, "beneficios": "Hasta 5 agendas, recordatorios WhatsApp, IA Cl├¡nica."},
            {"id": "Premium", "nombre": "Plan Premium (Anual)", "precio": 499.99, "duracion_dias": 365, "beneficios": "Ilimitado, IA avanzada, anal├¡tica predictiva, soporte 24/7."}
        ]
        return Response(catalogo, status=status.HTTP_200_OK)


class OnboardingSaaSAPIView(APIView):
    """POST p├║blico: alta at├│mica de Cl├¡nica + Administrador con selecci├│n de Plan."""
    permission_classes = [AllowAny]
    throttle_scope = "registro"
    throttle_classes = [ScopedRateThrottle]
...
class MiClinicaRetrieveUpdateAPIView(generics.RetrieveUpdateAPIView):
    """GET/PUT/PATCH: Ver y actualizar datos de la cl├¡nica del usuario logueado."""
    ...
    def perform_update(self, serializer):
        if self.request.user.rol != 'ADMIN':
            ...
            raise PermissionDenied("Solo el administrador puede modificar los datos de la cl├¡nica.")
        serializer.save()


class ClinicaCreateAPIView(APIView):
    """Alta de cl├¡nica adicional: solo rol ADMIN con cl├¡nica asignada."""
    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            serializer.save()
            return Response(
                {"message": "Cl├¡nica registrada"}, status=status.HTTP_201_CREATED
            )
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
...
class UsuarioColaboradorListCreateAPIView(generics.ListCreateAPIView):
    """Lista psic├│logos/usuarios de la misma cl├¡nica; POST crea colega (rol PSICOLOGO)."""
...
class UsuarioAdminRetrieveUpdateDestroyAPIView(generics.RetrieveUpdateDestroyAPIView):
    """GET/PUT/PATCH/DELETE de un usuario de la misma cl├¡nica (solo ADMIN)."""
...
# ==========================================================================
# CRUD de psic├│logos (T017) ÔÇö endpoints para web y m├│vil
# ==========================================================================
class PsicologoListCreateAPIView(generics.ListCreateAPIView):
    """GET: psic├│logos de la cl├¡nica. POST: alta de nuevo psic├│logo."""
    ...
    def get_queryset(self):
        ...
        # Si se pasa un clinica_id expl├¡cito (ej. desde el m├│vil del paciente)
        if clinica_id:
        ...
        # Fallback al comportamiento original (para el dashboard web admin/psic├│logo)
        return Usuario.objects.filter(
    ...
    def perform_create(self, serializer):
        clinica = self.request.user.clinica
        # SaaS Feature-Gating: No hay l├¡mites de psic├│logos en ning├║n plan.
        serializer.save(clinica=clinica, rol="PSICOLOGO")


class SuscripcionInfoAPIView(APIView):
    """GET: Devuelve el estado de la suscripci├│n y uso de cupos de la cl├¡nica."""
    ...
    def get(self, request, pk):
        ...
        if str(clinica.id) != str(pk):
            from rest_framework.exceptions import PermissionDenied
            raise PermissionDenied("No tienes acceso a esta suscripci├│n.")
        ...
        info = {
            ...
            "features": {
                "soporte": "Est├índar" if plan == "Basico" else "Prioritario 24h" if plan == "Profesional" else "VIP Dedicado 24/7",
...
class TransaccionClinicaListAPIView(generics.ListAPIView):
    """GET: Historial de transacciones de facturaci├│n de la cl├¡nica."""
...
class CargarSaldoAPIView(APIView):
    """POST: Cargar saldo a la cuenta de la cl├¡nica."""
    permission_classes = [IsAuthenticated, EsAdministrador, HasClinicaAsignada]
    ...
    def post(self, request):
        ...
        if not monto or float(monto) <= 0:
            return Response({"detail": "Monto inv├ílido."}, status=status.HTTP_400_BAD_REQUEST)
...
class PsicologoRetrieveUpdateDestroyAPIView(generics.RetrieveUpdateDestroyAPIView):
    """GET/PUT/PATCH/DELETE de un psic├│logo de la misma cl├¡nica."""
...
class NotificacionesMobileAPIView(APIView):
    """GET: Historial de notificaciones. PATCH: Marcar como le├¡das."""
    permission_classes = [IsAuthenticated]
    ...
    def patch(self, request):
        ...
        return Response({"mensaje": "Todas las notificaciones marcadas como le├¡das."}, status=status.HTTP_200_OK)
    ...
    def delete(self, request):
        ...
        if notif_id:
            try:
                ...
                return Response({"mensaje": "Notificaci├│n borrada."}, status=status.HTTP_200_OK)
