...
class _PacienteDashboardState extends State<PacienteDashboard> with WidgetsBindingObserver {
  ...
  List<dynamic> get _filteredClinicas {
    if (_searchQuery.isEmpty) return _clinicas;
    return _clinicas.where((c) {
            ...
            actions: [
              ...
              PopupMenuButton<String>(
                ...
                itemBuilder: (BuildContext context) => <PopupMenuEntry<String>>[
                  ...
                  PopupMenuItem<String>(
                    value: 'historial',
                    child: Row(children: [Icon(Icons.history, color: primaryBlue, size: 20), const SizedBox(width: 10), Text('Historial Médico', style: GoogleFonts.outfit())]),
                  ),
                  PopupMenuItem<String>(
                    value: 'pagos',
                    ...
                    children: [
                      ...
                      TextField(
                        decoration: InputDecoration(
                          hintText: 'Buscar clínica por nombre...',
                      ...
                      // Mostramos DINÁMICAMENTE la lista de clínicas filtradas
                      if (_filteredClinicas.isNotEmpty)
                        ..._filteredClinicas.map((clinica) => Padding(
                          ...
                          child: _buildClinicCard(
                            ...
                            horario: clinica['horarios_atencion'] ?? clinica['horario'] ?? 'Lunes a Viernes',
              ...
              Navigator.push(
                ...
                MaterialPageRoute(
                  builder: (context) => ChatbotScreen(
                    token: widget.token,
                    contextType: ChatbotContextType.global,
                    title: 'Asistente Global',
                  ),
                ),
              );
            },
            icon: const Icon(Icons.smart_toy, color: Colors.white),
            label: Text("Chatsito", style: GoogleFonts.outfit(color: Colors.white, fontWeight: FontWeight.bold)),
            backgroundColor: primaryBlue,
          ),
        ),
      ),
                ...
                child: OutlinedButton(
                  onPressed: () {
                    Navigator.push(
                      context,
                      MaterialPageRoute(
                        builder: (context) => ConsultarCitasScreen(
                          token: widget.token,
              ...
              onPressed: () {
                Navigator.push(
                  ...
                  MaterialPageRoute(
                    builder: (context) => ChatbotScreen(
                      token: widget.token,
                      contextType: ChatbotContextType.clinica,
                      contextId: clinicaId,
                      title: 'Asistente $nombre',
                    ),
                  ),
                );
              },
              icon: const Icon(Icons.smart_toy, color: Colors.white, size: 18),
              label: Text('Asistente Inteligente', style: GoogleFonts.outfit(color: Colors.white, fontWeight: FontWeight.w600)),
              style: ElevatedButton.styleFrom(
