...
class _AgendarCitaStepperScreenState extends State<AgendarCitaStepperScreen> {
  ...
  Future<void> _loadPsicologos() async {
    try {
      final psicologos = await CitaPagoService.getPsicologos(widget.token, clinicaId: widget.clinicaId);
      if (mounted) {
        setState(() {
  ...
  Future<void> _loadDisponibilidad() async {
    ...
    try {
      ...
      _showError('No se pudo obtener la disponibilidad.');
    }
  }
      ...
      child: Column(
        ...
        children: [
          if (_currentStep == 0) ...[
            ...
            else if (_psicologos.isEmpty)
              Text('No hay psicólogos disponibles en esta clínica actualmente.', style: GoogleFonts.outfit(color: Colors.red))
            else
              ..._psicologos.map((p) {
                final displayStr = '${p["username"]} - ${p["first_name"]} ${p["last_name"]}';
                return Container(
                  margin: const EdgeInsets.only(bottom: 12),
                  decoration: BoxDecoration(
                    border: Border.all(color: _selectedPsicologo == displayStr ? primaryBlue : Colors.grey.shade200, width: _selectedPsicologo == displayStr ? 2 : 1),
                    borderRadius: BorderRadius.circular(12),
                    color: _selectedPsicologo == displayStr ? primaryBlue.withOpacity(0.05) : Colors.transparent,
                  ),
                  child: RadioListTile<String>(
                    title: Text(displayStr, style: GoogleFonts.outfit(fontWeight: FontWeight.w500)),
                    value: displayStr,
              ...
              decoration: InputDecoration(
                ...
                suffixIcon: IconButton(
                  icon: _isProcessingVoice 
                      ...
                      : Icon(_isRecording ? Icons.stop : Icons.mic, color: _isRecording ? Colors.red : const Color(0xFF2563EB)),
  ...
  Widget _buildTimeBtn(String time) {
    ...
    bool isOccupied = _horasOcupadas.contains(time);
    ...
    return InkWell(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          color: isOccupied ? Colors.grey.shade300 : (isSelected ? primaryBlue : Colors.white),
          borderRadius: BorderRadius.circular(12),
          border: Border.all(color: isOccupied ? Colors.grey.shade400 : (isSelected ? primaryBlue : Colors.grey.shade300)),
        ...
        child: Center(
          child: Text(
            time,
            style: GoogleFonts.outfit(
              color: isOccupied ? Colors.grey.shade500 : (isSelected ? Colors.white : darkBlue),
              fontWeight: FontWeight.bold,
