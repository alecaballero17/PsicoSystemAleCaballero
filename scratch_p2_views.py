...
def registrar_paciente_view(request):
    if not getattr(request.user, "clinica", None):
        messages.error(
            request,
            "ATENCI├ôN: Tu cuenta actual no est├í asignada a ninguna cl├¡nica. No puedes registrar pacientes hasta tener una cl├¡nica.",
        )
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            LogAuditoria.objects.create(
                ...
                accion=f"Registr├│ un nuevo paciente: {paciente.nombre}",
            )
            return redirect("dashboard")
    else:
        form = PacienteForm()
...
def editar_paciente_view(request, pk):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            LogAuditoria.objects.create(
                usuario=request.user,
                accion=f"Actualiz├│ paciente: {paciente.nombre}",
            )
...
class PacienteListCreateAPIView(generics.ListCreateAPIView):
    ...
    def perform_create(self, serializer):
        ...
        LogAuditoria.objects.create(
            ...
            accion=f"Registr├│ un nuevo paciente (API): {paciente.nombre}",
        )
...
class PacienteRetrieveUpdateAPIView(generics.RetrieveUpdateDestroyAPIView):
    ...
    def perform_update(self, serializer):
        ...
        LogAuditoria.objects.create(
            usuario=self.request.user,
            accion=f"Actualiz├│ paciente (API): {paciente.nombre}",
        )

# ==============================================================================
# T027: B├║squeda y Filtrado
# ==============================================================================
...
# ==============================================================================
# T026: API de Expediente Cl├¡nico
# ==============================================================================
...
# ==============================================================================
# T015: Registro P├║blico M├│vil (App) y Vinculaci├│n
# ==============================================================================
import logging
